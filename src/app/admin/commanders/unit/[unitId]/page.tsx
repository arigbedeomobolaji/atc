      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          <p className="text-sm font-bold text-slate-800">
            {abbrevRank(c.rank)} {c.name}
          </p>
          {isCurrent && (
            <span className="px-2 py-0.5 rounded-full bg-[hsl(45,68%,47%)] text-[hsl(220,64%,16%)] text-[10px] font-black uppercase tracking-wider">
              Current
            </span>
          )}
        </div>
        <div className="flex items-center gap-3 mt-0.5 text-xs text-slate-400">
          <span className="flex items-center gap-1">
            <CalendarDays size={10} /> {formatDate(c.startDate)}
          </span>
...
export default function UnitCommandersPage() {
  ...
  const [pastCommander, setPastCommander] = useState<any[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      const [cmdRes, currentCmdRes, unitRes] = await Promise.all([
        fetch(`/api/commanders/history/${unitId}`),
        fetch(`/api/commanders/current/${unitId}`),
        fetch(`/api/units/${unitId}`),
      ]);
      setPastCommander(await cmdRes.json());
      setCurrentCommander(await currentCmdRes.json());
  ...
  async function handleDelete(id: string) {
    if (!confirm("Delete this commander record? This cannot be undone."))
      return;
    const res = await fetch(`/api/commanders/${id}`, { method: "DELETE" });
    if (res.ok) {
      setPastCommander((prev) => prev.filter((c) => c._id !== id));
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm mt-0.5">
              Commander records ·{" "}
              {pastCommander.length + (currentCommander ? 1 : 0)} total
            </p>
          </div>
        </div>
          ...
          <div className="flex items-center gap-4 px-5 py-4 rounded-xl border-2 border-dashed border-slate-200 text-slate-400">
            ...
            <div>
              <p className="text-sm font-medium">
                No current commander assigned
              </p>
              <Link
                href={`/admin/commanders/create?unitId=${unitId}`}
                className="text-xs text-[hsl(220,64%,16%)] hover:underline mt-0.5 inline-block"
              >
                Assign one now →
              </Link>
            </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-slate-300">
            ({pastCommander.length})
          </span>
        </div>
        {pastCommander.length > 0 ? (
          pastCommander.map((c) => (
            <CommanderCard key={c._id} c={c} onDelete={handleDelete} />
          ))
        ) : (
          <p className="px-5 py-4 text-sm text-slate-400 border border-dashed border-slate-200 rounded-xl">
            No past commander records.
          </p>
        )}
      </div>
    </div>
