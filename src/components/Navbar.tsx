...
const navItems = [
  ...
  // NEWS DROPDOWN
  {
    ...
    href: "/news",
    adminOnlyChildren: [
      { label: "Create News", href: "/dashboard/news/create" },
      { label: "Manage News", href: "/dashboard/news" },
    ],
    children: [{ label: "News Feed", href: "/news" }],
  },
        ...
        <motion.div className="font-link text-[16px] font-medium hidden md:flex gap-5 lg:gap-14">
          {navItems.map((item, index) => {
            const parentActive =
              ...
              isParentActive(item.href, item.adminOnlyChildren);

            const desktopChildren = [
              ...(item.children || []),
              ...(isAdmin ? item.adminOnlyChildren || [] : []),
            ];
